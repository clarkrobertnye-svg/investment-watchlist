...
def build_universe():
    """
    Two-step universe build:
    1. Pull full stock list, filter to likely US large caps
    2. Check market cap for each via market-capitalization endpoint
    3. Cache result to universe_cache.json for reuse
    """
    cache_path = os.path.join(OUTPUT_DIR, "universe_cache.json")
    ...
    if os.path.exists(cache_path):
        ...
        if age_days < 7:
            with open(cache_path) as f:
                cached = json.load(f)
            print(f"Using cached universe ({len(cached)} tickers, {age_days:.1f} days old)")
            for t in ["NVDA","AAPL","GOOGL","V","MA","MSFT","AZO","ADP"]:
                print(f"    {t}: {'ok' if t in cached else 'MISSING'}")
            print()
            return cached
    
    print("Building universe from FMP (this takes ~10 min, cached for 7 days)...")
    
    # Step 1: Get all symbols
    print("  Step 1: Pulling stock list...")
    stock_list = fetch_json(f"{BASE_URL}/stock-list?apikey={API_KEY}")
    if not stock_list or not isinstance(stock_list, list):
        print("  x stock-list failed."); sys.exit(1)
    print(f"    Total symbols: {len(stock_list)}")
    
    # Filter to likely US-listed equities
    candidates = []
    for s in stock_list:
        sym = s.get("symbol", "")
        if not sym: continue
        if not sym.isalpha(): continue                       # alpha only
        if len(sym) > 5: continue                            # skip long symbols
        if len(sym) == 5 and sym[-1] in ('F', 'Y'): continue  # OTC foreign
        if len(sym) >= 4 and sym[-1] == 'X': continue       # mutual funds (FHANX etc)
        if sym[-1] in ('Q', 'W'): continue                  # delisted / warrants
        if len(sym) == 5 and sym[-1] not in ('L','K','A','B'): continue  # most 5-char large caps end in L,K,A,B (GOOGL, LSXMK, FOXA)
        candidates.append(sym)
    
    print(f"    After symbol filter: {len(candidates)} candidates")
    
    # Step 2: Check market cap for each (fast — lightweight endpoint)
    print(f"  Step 2: Checking market caps (~{len(candidates)*0.05/60:.0f} min, cached for 7 days)...")
    ...
    for i, sym in enumerate(candidates):
        if (i+1) % 1000 == 0:
            ...
            rate = (i+1) / elapsed
            remaining = (len(candidates) - i - 1) / rate / 60
            print(f"    ... {i+1}/{len(candidates)} checked, {len(universe)} above $10B, ~{remaining:.0f} min remaining")
        ...
        try:
            ...
            if mc >= 10_000_000_000:
                universe.append({"symbol": sym, "mktCap": mc})
        except:
            continue
    
    elapsed = time.time() - start_time
    print(f"    Done: {len(candidates)} checked in {elapsed/60:.1f} min, {len(universe)} above $10B")
    
    # Deduplicate dual-class
    sym_set = {s["symbol"] for s in universe}
    remove = set()
    for drop, keep in [("GOOG","GOOGL"),("FOX","FOXA"),("NWS","NWSA")]:
        if drop in sym_set and keep in sym_set: remove.add(drop)
    universe = [s for s in universe if s["symbol"] not in remove]
    ...
    # Sanity check
    for t in ["NVDA","AAPL","GOOGL","V","MA","MSFT","AZO","ADP"]:
        print(f"    {t}: {'ok' if t in tickers else 'MISSING'}")
    print()
...
def process_ticker(symbol):
    ...
    if profile:
        result["mktCap"] = profile.get("marketCap",0) or 0
    ...
    for i in range(n):
        ...
        dividends = cf.get("commonDividendsPaid",0) or 0
...
def main():
    ...
    log_path = os.path.join(OUTPUT_DIR, "compounders_v3_log.txt")
    csv_path = os.path.join(OUTPUT_DIR, "compounders_v3_results.csv")
    all_data_path = os.path.join(OUTPUT_DIR, "compounders_v3_all_data.json")
